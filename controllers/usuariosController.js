...
var controller = {
    ...
    usuario: function (req, res){
        ...
        Usuarios.findOne({usuario_id:  filtro}).exec((err, usuario)=>{
            if (err) return res.status(500).json({ status: 500,  mensaje: err  });
            if (!usuario) return res.status(200).json({ status: 200,  mensaje: "No existe el usuario" });
            console.log(usuario);
            return res.status(200).json({
                status: 200,
                data: usuario
    ...
    crear_usuario: function (req, res){
        ...
        Usuarios.findOne({usuario_id:  info_usuario.usuario_id}).exec((err, usuario)=>{
            ...
            const new_hash = bcrypt.hashSync(info_usuario.password, bc_salt_round);
            console.log(new_hash);
            usuario_model.password = new_hash;
